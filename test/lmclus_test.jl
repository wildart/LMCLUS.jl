;cd /Users/Artyom\ Diky/Documents/GitHub/LMCLUS.jl/src
include("LMCLUS.jl")
using LMCLUS
using Base.Test
...
p.heuristic = 2
@test LMCLUS.sample_quantity(1,3,1000,p) == 3
p.heuristic = 3
@test LMCLUS.sample_quantity(1,3,1000,p) == 3

# Test data sampling
@test size(unique(LMCLUS.randperm2(10,5)),1) == 5
@test size(LMCLUS.sample_points(eye(5, 3),3),1) == 4
@test_throws size(LMCLUS.sample_points(eye(5, 3),4),1) == 4
...
data[1,3] = 1
@test_throws LMCLUS.form_basis(data)
